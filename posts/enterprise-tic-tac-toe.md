...
layout: post
title: "エンタープライズ三目並べ"
description: "純粋関数型で実装するときの設計ガイド"
categories: ["実践例"]
...
今回は、三目並べ (○×ゲーム) を題材に、関数型スタイルで実装する方法をステップごとに見ていきましょう。
...
最も分かりやすいのは、整数でインデックス化された2次元グリッドを使う方法でしょう。例えば、`(1,1)`、`(1,2)`、`(1,3)`のように。
...
UIは、例えば空のボタンを並べたグリッドを作成し、各ボタンにセルを関連付けて、「空」の状態を描画します。
...
例えば、`public` と `private` の修飾子を正しく使い分けようとすると、次のようなコンパイルエラーが発生しやすくなります。
...
しかし、もし `GameState` を継承した実装クラスが2つあった場合、例えば、実装Bのゲーム状態を実装Aのゲーム状態を期待している関数に渡してしまう、といったことが起こりえます。
