...
**和** と **積** が実際には何を意味するのかについては、後ほどこのシリーズでタプル (積) と 判別共用体 (和) 型について詳しく議論するときに説明します。
...
特に、和型または「共用体」は非常に価値があり、慣れればなくてはならないものになるでしょう。
...
<td>
常に使えて、<code>type</code>キーワードで明示的に定義しない。
使用はカンマで示す（かっこはオプション）。
</td>
</tr>
<tr>
...
<td>
コンストラクタとして使う関数スタイルのパラメータリストが名前の後に続く。<br>
"member" キーワードを使う。<br>
セカンダリコンストラクタには "new" キーワードを使う。
</td>
</tr>
<tr>
<td>
...
抽象メンバーは具体的な実装ではなく、コロンと型シグネチャを持つ。
</td>
</tr>
<tr>
<td>
