...
* コードブロックを区切るために、波括弧 ({ }) を使わない。代わりに、インデントを使う (Python もこの点で似ている)。
...
let square x = x * x          // 括弧 ( ) を使用しないことに注意
square 3                      // 関数を呼び出してみよう。ここでも括弧は不要
...
// 括弧を使って優先順位を明確にすることができる
// この例では、まず "map" を 2 つの引数で実行し、その後その結果に対して "sum" を実行する
// 括弧がない場合、"List.map" が List.sum に引数として渡されてしまう
