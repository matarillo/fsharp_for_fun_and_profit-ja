...
.NETライブラリとの接続時にこのようなエラーが発生することがよくあります。例えば、 `TextReader` の `ReadLine` メソッドは unit パラメータを取る必要があります。これを忘れ、かっこを省略してしまうことはよくあることで、その場合、コンパイルエラーはすぐに発生せず、結果を文字列として扱おうとしたときに初めてエラーになります。
