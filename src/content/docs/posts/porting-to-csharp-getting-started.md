...
移植を始める前に、F#の構文がC#の構文とどのように異なるかを理解する必要があります。このセクションでは、一方から他方への変換に関する一般的なガイドラインを示します。（F#構文の簡単な概要については、「[60秒でわかるF#構文](../posts/fsharp-in-60-seconds.html)」をご覧ください）
...
インデントルールの詳細については、[この投稿](../posts/fsharp-syntax.html)を参照してください。
...
カンマは一般的に、タプルや.NETライブラリ関数を呼び出す際のパラメータの区切りにのみ使われます。（タプルと複数パラメータの違いについては、[この投稿](../posts/defining-functions.html#tuples)を参照してください）
...
誤って`!=`を使用すると、おそらく[error FS0020](../troubleshooting-fsharp.html#FS0020)が発生します。
...
より複雑なクラス定義は次の例で示します。詳細な解説は、[クラスに関する完全な説明](../posts/classes.html)をご覧ください。
...
式指向のコードの詳細については、「[式 vs. 文](../posts/expressions-vs-statements.html)」を参照してください。

### コンソールへの出力

C#で出力を行うには、一般的に`Console.WriteLine`やそれに類似したものを使用します。F#では、一般的に型安全な`printf`やそれに類似したものを使用します。（[「printf」ファミリーの使用に関する詳細](../posts/printf.html)）
