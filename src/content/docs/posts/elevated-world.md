...
[このような内容を書かないと約束した](../posts/why-i-wont-be-writing-a-monad-tutorial.html)のは承知していますが、
今回は少し違うアプローチを試みました。
...
[プロパティベースのテストに関する私の投稿](https://fsharpforfunandprofit.com/pbt/)で説明したように、関数の正しい実装は、特定の例ではなく一般的な特性を使って定義し、テストすることができます。
...
* **世界をまたぐ関数を扱うmap**。map関数`a->b`は完全に通常の世界に属しています。しかし、マッピングしたい関数が
  通常の世界に戻らず、別の高次の世界の値を返す場合はどうでしょうか？ この課題への対処方法は[後の投稿](../posts/elevated-world-4.html)で見ていきます。
...
詳しい情報は、「[つらくないモノイド](../posts/monoids-without-tears.html)」に関する私の投稿をご覧ください。
...
[次の投稿](../posts/elevated-world-2.html)では、これらの世界をまたぐ関数も高次の世界に持ち上げる方法を紹介します。
